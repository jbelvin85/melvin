...
- A lightweight mana analyzer parses statements like “I control two Islands” and, using the tagged card list and Oracle mana costs, determines whether the player has enough mana to cast the requested spells. Its verdict is appended to the `tools_context` and mentioned in the reasoning drawer so Melvin can cite it (“Mana analyzer → Result: NOT enough mana”).
- The sequencing planner (`backend/app/services/sequencer.py`) explores different casting orders for the tagged cards (up to four spells) using the parsed mana pool. For each playable order it reports post-cast resources (e.g., “Order 'Sol Ring → Arcane Signet' playable, leftover mana C:2.”). These findings show up in the insight drawer and give the LLM concrete facts about optimal sequencing.
- Card-type checks run alongside the knowledge lookups. For every tagged card, we emit deterministic facts such as “CardType: Sol Ring is Artifact (nonland, cast via CR 601)” or “CardType: Dryad Arbor is Land Creature (play only as a land per CR 305).” This ensures Melvin never infers card types from memory; instead he cites the tool output verbatim. Cards whose abilities depend on commander color identity now trigger explicit warnings when the user hasn’t told us their commander’s colors, and we persist declared colors (e.g., “Commander identity ⇒ blue-red”) for future responses. Add new judge scenarios (e.g., “two Islands, can I cast [Sol Ring] and [Arcane Signet]?”) to the evaluation harness so regressions are caught automatically.
- A lightweight evaluation harness lives at `backend/app/services/eval_harness.py`. Run it via `./scripts/melvin.sh eval` (which executes `python -m app.services.eval_harness` inside the API container) after deployments to hit a curated set of judge-style prompts and confirm that every response includes citations, warnings for unknown cards/rules, and a non-empty context snapshot. Expand `SAMPLE_CASES` with real judge calls as our corpus grows.
