...
- `scripts/melvin.sh` now orchestrates the entire lifecycle:
  - `launch` (default): verifies raw dataset files (prompts for their local paths if missing), prompts for admin credentials, generates the JWT secret, ensures the configured Ollama model is pulled (invoking `ollama pull <model>` if needed), builds the React frontend, brings up Docker services, waits for the API health check, and prints the access URL plus admin username.
